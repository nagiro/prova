<?php
     ...
     * */
    function ph_getEtiquetaCursos($AUTEN, $JaMat, $TReserva, $TReservaT, $TNReserva, $url, $idS = 1, $datai = null)
    {
        $RET = "";                                        
        ...
        if( $AUTEN && $TNReserva ){
            ...
            $RET .= ph_getRoundCorner('Matrícula presencial', '#FF0000').'</div>';
        }elseif($avui < $datai){
            $OS = SitesPeer::retrieveByPK($idS);
            if(!($OS instanceof Sites)) $OS = new Sites();
            $tel  = $OS->getTelefonString(); $email = $OS->getEmailString(); $nom = $OS->getNom();                        
        ...
        }elseif( $AUTEN && !$JaMat && ($TReserva || $TReservaT )){
