<?php use_helper('Form') ?>
	...
	<?php IF($MODE['CONSULTA']): ?>
     ...
     <DIV class="REQUADRE">
        <DIV class="TITOL">Llistat d'usuaris (<?=$PAGER_USUARIS->getNbResults()?>)</DIV>
      	<TABLE class="DADES">
                ...
                <? else: ?>                
	                <? foreach($PAGER_USUARIS->getResults() as $U): ?>
	                 	<TR><TD class="LINIA"><?=link_to($U->getDni(),'gestio/gUsuaris'.getPar($PAGINA,$U->getusuariid(),'E'))?></TD>
	                        <TD class="LINIA"><?=$U->getNomComplet()?></TD>
	                        <TD class="LINIA"><?=$U->getTelefon()?></TD> 
	                        <TD class="OPCIONS"><?=creaOpcions($PAGINA, $U->getusuariid(), NULL)?></TD>
	                    </TR>                                   
	                <? endforeach; ?>                     
	                	<TR><TD><?=gestorPaginesUsuaris( $PAGER_USUARIS )?></TD></TR>
	            <? endif; ?>
      	</TABLE>      
      </DIV>
	    	...
	    	<table class="FORMULARI" width="500px">
                ...
                <tr>
	            	...
	            	<td class="dreta" width="400px">
	            		...
	            		<?=link_to(image_tag('icons/Colored/PNG/action_delete.png'),'gestio/gUsuaris',array('confirm'=>'Segur que vols esborrar-lo?'))?>
	            	</td>
	            </tr>                	 
      		</TABLE>
      	</DIV>
