 ...
 */
class sfWidgetFormInputFileEditableMy extends sfWidgetFormInputFile
{
  /**
   * Constructor.
   *
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
	...
	if(is_null($value) || empty($value)): $file = 'No hi ha cap arxiu.'; else: $file = $this->getFileAsTag($attributes); endif;  
    
    return strtr($this->getOption('template'), array('%input%' => $input, '%delete%' => $delete, '%delete_label%' => $deleteLabel, '%file%' => $file));
  }
  ...
  protected function getFileAsTag($attributes)
  {
    ...
    {
      return false !== $this->getOption('file_src') ? $this->renderTag('img', array_merge(array('src' => $this->getOption('file_src'),'width'=>'100px')), $attributes) : '';
    }
    else
    {
