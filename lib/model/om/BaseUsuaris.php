...
abstract class BaseUsuaris extends BaseObject  implements Persistent {
  ...
  const PEER = 'UsuarisPeer';
	 ...
	 * related Clubers from storage. If this Usuaris is new, it will return
		...
		if ($this->collClubers === null) {
			if ($this->isNew()) {
			   $this->collClubers = array();
				...
				$criteria->add(CluberPeer::USUARIS_USUARIID, $this->usuariid);
	 ...
	 * actually need in Usuaris.
	 */
	public function getClubersJoinEntitats($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsuarisPeer::DATABASE_NAME);
		}
		...
		if ($this->collClubers === null) {
			if ($this->isNew()) {
				...
				$this->collClubers = CluberPeer::doSelectJoinEntitats($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
