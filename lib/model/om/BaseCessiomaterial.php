...
abstract class BaseCessiomaterial extends BaseObject  implements Persistent {
	...
	public function setCessioId($v)
	{
		...
		if ($this->cessio_id !== $v) {
			$this->cessio_id = $v;
			$this->modifiedColumns[] = CessiomaterialPeer::CESSIO_ID;
		}

		if ($this->aCessio !== null && $this->aCessio->getCessioId() !== $v) {
			$this->aCessio = null;
		}
	...
	public function setSiteId($v)
	{
		...
		if ($this->site_id !== $v) {
			$this->site_id = $v;
			$this->modifiedColumns[] = CessiomaterialPeer::SITE_ID;
		}
	...
	public function setActiu($v)
	{
		...
		if ($this->actiu !== $v || $this->isNew()) {
			$this->actiu = $v;
			$this->modifiedColumns[] = CessiomaterialPeer::ACTIU;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->cessio_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			...
			return $startcol + 5; // 5 = CessiomaterialPeer::NUM_COLUMNS - CessiomaterialPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCessio !== null && $this->cessio_id !== $this->aCessio->getCessioId()) {
			$this->aCessio = null;
		}
	} // ensureConsistency
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aCessio !== null) {
				if ($this->aCessio->isModified() || $this->aCessio->isNew()) {
					$affectedRows += $this->aCessio->save($con);
				}
				$this->setCessio($this->aCessio);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aCessio !== null) {
				if (!$this->aCessio->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aCessio->getValidationFailures());
				}
			}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setCessioId($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CessiomaterialPeer::CESSIO_ID)) $criteria->add(CessiomaterialPeer::CESSIO_ID, $this->cessio_id);
		if ($this->isColumnModified(CessiomaterialPeer::SITE_ID)) $criteria->add(CessiomaterialPeer::SITE_ID, $this->site_id);
		if ($this->isColumnModified(CessiomaterialPeer::ACTIU)) $criteria->add(CessiomaterialPeer::ACTIU, $this->actiu);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCessioId($this->cessio_id);
