...
abstract class BaseCessiomaterial extends BaseObject  implements Persistent {
  ...
  const PEER = 'CessiomaterialPeer';
	...
	/**
	 * The value for the estatretornat field.
	 * @var        string
	 */
	protected $estatretornat;

	/**
	 * The value for the dataretornat field.
	 * @var        string
	 */
	...
	/**
	 * Get the [estatretornat] column value.
	 * 
	 * @return     string
	 */
	public function getEstatretornat()
	{
		return $this->estatretornat;
	}

	/**
	 * Get the [optionally formatted] temporal [dataretornat] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->dataretornat === null) {
			return null;
		}


		if ($this->dataretornat === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->dataretornat);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->dataretornat, true), $x);
			}
		}
	...
	/**
	 * Set the value of [estatretornat] column.
	 * 
	 * @param      string $v new value
	 * @return     Cessiomaterial The current object (for fluent API support)
	 */
	public function setEstatretornat($v)
	{
		...
		if ($this->estatretornat !== $v) {
			$this->estatretornat = $v;
			$this->modifiedColumns[] = CessiomaterialPeer::ESTATRETORNAT;
		}
	...
	/**
	 * Sets the value of [dataretornat] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Cessiomaterial The current object (for fluent API support)
	...
	public function setDataretornat($v)
	{
		...
		if ( $this->dataretornat !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->dataretornat !== null && $tmpDt = new DateTime($this->dataretornat)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->dataretornat = ($dt ? $dt->format('Y-m-d') : null);
				$this->modifiedColumns[] = CessiomaterialPeer::DATARETORNAT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->estatretornat = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->dataretornat = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->resetModified();
			...
			return $startcol + 9; // 9 = CessiomaterialPeer::NUM_COLUMNS - CessiomaterialPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[7] => $this->getEstatretornat(),
			$keys[8] => $this->getDataretornat(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[7], $arr)) $this->setEstatretornat($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setDataretornat($arr[$keys[8]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CessiomaterialPeer::ESTATRETORNAT)) $criteria->add(CessiomaterialPeer::ESTATRETORNAT, $this->estatretornat);
		if ($this->isColumnModified(CessiomaterialPeer::DATARETORNAT)) $criteria->add(CessiomaterialPeer::DATARETORNAT, $this->dataretornat);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setEstatretornat($this->estatretornat);

		$copyObj->setDataretornat($this->dataretornat);
