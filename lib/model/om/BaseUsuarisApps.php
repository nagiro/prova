...
/**
 * Base class that represents a row from the 'usuaris_apps' table.
 *
 * 
 *
 ...
 */
abstract class BaseUsuarisApps extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseUsuarisApps object.
	 * @see        applyDefaults()
	 */
	...
	public function setUsuariId($v)
	{
		...
		if ($this->usuari_id !== $v) {
			$this->usuari_id = $v;
			$this->modifiedColumns[] = UsuarisAppsPeer::USUARI_ID;
		}
	...
	public function setAppId($v)
	{
		...
		if ($this->app_id !== $v) {
			$this->app_id = $v;
			$this->modifiedColumns[] = UsuarisAppsPeer::APP_ID;
		}

		if ($this->aApps !== null && $this->aApps->getAppId() !== $v) {
			$this->aApps = null;
		}
	...
	public function setNivellId($v)
	{
		...
		if ($this->nivell_id !== $v) {
			$this->nivell_id = $v;
			$this->modifiedColumns[] = UsuarisAppsPeer::NIVELL_ID;
		}
	...
	public function setSiteId($v)
	{
		...
		if ($this->site_id !== $v) {
			$this->site_id = $v;
			$this->modifiedColumns[] = UsuarisAppsPeer::SITE_ID;
		}
	...
	public function setActiu($v)
	{
		...
		if ($this->actiu !== $v || $this->isNew()) {
			$this->actiu = $v;
			$this->modifiedColumns[] = UsuarisAppsPeer::ACTIU;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = UsuarisAppsPeer::NUM_COLUMNS - UsuarisAppsPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating UsuarisApps object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aApps !== null && $this->app_id !== $this->aApps->getAppId()) {
			$this->aApps = null;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsuarisAppsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseUsuarisApps:delete:pre') as $callable)
			{
			...
			if ($ret) {
				UsuarisAppsPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsuarisApps:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseUsuarisApps:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsuarisApps:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				UsuarisAppsPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aApps !== null) {
				if ($this->aApps->isModified() || $this->aApps->isNew()) {
					$affectedRows += $this->aApps->save($con);
				}
				$this->setApps($this->aApps);
			}
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UsuarisAppsPeer::doInsert($this, $con);
					...
					$affectedRows += UsuarisAppsPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aApps !== null) {
				if (!$this->aApps->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aApps->getValidationFailures());
				}
			}
			...
			if (($retval = UsuarisAppsPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UsuarisAppsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarisAppsPeer::USUARI_ID)) $criteria->add(UsuarisAppsPeer::USUARI_ID, $this->usuari_id);
		if ($this->isColumnModified(UsuarisAppsPeer::APP_ID)) $criteria->add(UsuarisAppsPeer::APP_ID, $this->app_id);
		if ($this->isColumnModified(UsuarisAppsPeer::NIVELL_ID)) $criteria->add(UsuarisAppsPeer::NIVELL_ID, $this->nivell_id);
		if ($this->isColumnModified(UsuarisAppsPeer::SITE_ID)) $criteria->add(UsuarisAppsPeer::SITE_ID, $this->site_id);
		if ($this->isColumnModified(UsuarisAppsPeer::ACTIU)) $criteria->add(UsuarisAppsPeer::ACTIU, $this->actiu);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UsuarisAppsPeer::USUARI_ID, $this->usuari_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsuarisApps (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsuarisApps Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsuarisAppsPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsuarisAppsPeer();
		}
		return self::$peer;
	}
	...
	public function getUsuaris(PropelPDO $con = null)
	{
		if ($this->aUsuaris === null && ($this->usuari_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aUsuaris->addUsuarisAppss($this);
			 */
		}
	...
	public function getNivells(PropelPDO $con = null)
	{
		if ($this->aNivells === null && ($this->nivell_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aNivells->addUsuarisAppss($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseUsuarisApps:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseUsuarisApps::%s', $method));
	  }
