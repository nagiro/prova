...
abstract class BaseEntradesPreus extends BaseObject  implements Persistent {
	...
	public function setPreur($v)
	{
		...
		if ($this->preur !== $v) {
			$this->preur = $v;
			$this->modifiedColumns[] = EntradesPreusPeer::PREUR;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->preur = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->places = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->tipus = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->activitat_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->descomptes = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getPreu(),
			$keys[2] => $this->getPreur(),
			$keys[3] => $this->getPlaces(),
			$keys[4] => $this->getTipus(),
			$keys[5] => $this->getActivitatId(),
			$keys[6] => $this->getDescomptes(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setPreur($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setPlaces($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setTipus($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setActivitatId($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setDescomptes($arr[$keys[6]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EntradesPreusPeer::PREUR)) $criteria->add(EntradesPreusPeer::PREUR, $this->preur);
