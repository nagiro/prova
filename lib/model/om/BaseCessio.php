  ...
  const PEER = 'CessioPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->representant = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->motiu = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->condicions = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->material_no_inventariat = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->data_cessio = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->data_retorn = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->estat = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->retornat = ($row[$startcol + 9] !== null) ? (int) $row[$startcol + 9] : null;
			$this->estat_retornat = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->data_retornat = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->resetModified();
			...
			return $startcol + 12; // 12 = CessioPeer::NUM_COLUMNS - CessioPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[2] => $this->getRepresentant(),
			$keys[3] => $this->getMotiu(),
			$keys[4] => $this->getCondicions(),
			$keys[5] => $this->getMaterialNoInventariat(),
			$keys[6] => $this->getDataCessio(),
			$keys[7] => $this->getDataRetorn(),
			$keys[8] => $this->getEstat(),
			$keys[9] => $this->getRetornat(),
			$keys[10] => $this->getEstatRetornat(),
			$keys[11] => $this->getDataRetornat(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setRepresentant($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setMotiu($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setCondicions($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setMaterialNoInventariat($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setDataCessio($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setDataRetorn($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setEstat($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setRetornat($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setEstatRetornat($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setDataRetornat($arr[$keys[11]]);
	}
	...
	public function countCessiomaterials(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collCessiomaterials === null) {
			if ($this->isNew()) {
				...
				$count = CessiomaterialPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
