...
abstract class BaseEntradesReserva extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tipus = ($row[$startcol + 14] !== null) ? (int) $row[$startcol + 14] : null;
			$this->descompte = ($row[$startcol + 15] !== null) ? (int) $row[$startcol + 15] : null;
			$this->tpv_operacio = ($row[$startcol + 16] !== null) ? (string) $row[$startcol + 16] : null;
			$this->tpv_order = ($row[$startcol + 17] !== null) ? (int) $row[$startcol + 17] : null;
			$this->comentari = ($row[$startcol + 18] !== null) ? (string) $row[$startcol + 18] : null;
			$this->resetModified();
			...
			return $startcol + 19; // 19 = EntradesReservaPeer::NUM_COLUMNS - EntradesReservaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[14] => $this->getTipus(),
			$keys[15] => $this->getDescompte(),
			$keys[16] => $this->getTpvOperacio(),
			$keys[17] => $this->getTpvOrder(),
			$keys[18] => $this->getComentari(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[14], $arr)) $this->setTipus($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setDescompte($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setTpvOperacio($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setTpvOrder($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setComentari($arr[$keys[18]]);
	}
