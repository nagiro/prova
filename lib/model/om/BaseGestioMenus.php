...
/**
 * Base class that represents a row from the 'gestio_menus' table.
 *
 * 
 *
 ...
 */
abstract class BaseGestioMenus extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseGestioMenus object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     GestioMenus The current object (for fluent API support)
	 */
	public function setMenuId($v)
	{
		...
		if ($this->menu_id !== $v) {
			$this->menu_id = $v;
			$this->modifiedColumns[] = GestioMenusPeer::MENU_ID;
		}
	...
	public function setTitol($v)
	{
		...
		if ($this->titol !== $v) {
			$this->titol = $v;
			$this->modifiedColumns[] = GestioMenusPeer::TITOL;
		}
	...
	public function setUrl($v)
	{
		...
		if ($this->url !== $v) {
			$this->url = $v;
			$this->modifiedColumns[] = GestioMenusPeer::URL;
		}
	...
	public function setCategoria($v)
	{
		...
		if ($this->categoria !== $v) {
			$this->categoria = $v;
			$this->modifiedColumns[] = GestioMenusPeer::CATEGORIA;
		}
	...
	public function setOrdre($v)
	{
		...
		if ($this->ordre !== $v) {
			$this->ordre = $v;
			$this->modifiedColumns[] = GestioMenusPeer::ORDRE;
		}
	...
	public function setActiu($v)
	{
		...
		if ($this->actiu !== $v) {
			$this->actiu = $v;
			$this->modifiedColumns[] = GestioMenusPeer::ACTIU;
		}
	...
	public function setTipus($v)
	{
		...
		if ($this->tipus !== $v || $this->isNew()) {
			$this->tipus = $v;
			$this->modifiedColumns[] = GestioMenusPeer::TIPUS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = GestioMenusPeer::NUM_COLUMNS - GestioMenusPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating GestioMenus object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GestioMenusPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseGestioMenus:delete:pre') as $callable)
			{
			...
			if ($ret) {
				GestioMenusPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGestioMenus:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseGestioMenus:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGestioMenus:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				GestioMenusPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = GestioMenusPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
										 // should always be true here (even though technically
										 // BasePeer::doInsert() can insert multiple rows).

					$this->setMenuId($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += GestioMenusPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = GestioMenusPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = GestioMenusPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setTipus($arr[$keys[6]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GestioMenusPeer::MENU_ID)) $criteria->add(GestioMenusPeer::MENU_ID, $this->menu_id);
		if ($this->isColumnModified(GestioMenusPeer::TITOL)) $criteria->add(GestioMenusPeer::TITOL, $this->titol);
		if ($this->isColumnModified(GestioMenusPeer::URL)) $criteria->add(GestioMenusPeer::URL, $this->url);
		if ($this->isColumnModified(GestioMenusPeer::CATEGORIA)) $criteria->add(GestioMenusPeer::CATEGORIA, $this->categoria);
		if ($this->isColumnModified(GestioMenusPeer::ORDRE)) $criteria->add(GestioMenusPeer::ORDRE, $this->ordre);
		if ($this->isColumnModified(GestioMenusPeer::ACTIU)) $criteria->add(GestioMenusPeer::ACTIU, $this->actiu);
		if ($this->isColumnModified(GestioMenusPeer::TIPUS)) $criteria->add(GestioMenusPeer::TIPUS, $this->tipus);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(GestioMenusPeer::MENU_ID, $this->menu_id);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     GestioMenus Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     GestioMenusPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new GestioMenusPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this GestioMenus has previously been saved, it will retrieve
	 * related UsuarisMenuss from storage. If this GestioMenus is new, it will return
	 ...
	 * Otherwise if this GestioMenus is new, it will return
	 * an empty collection; or if this GestioMenus has previously
	 * been saved, it will retrieve related UsuarisMenuss from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in GestioMenus.
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseGestioMenus:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseGestioMenus::%s', $method));
	  }
